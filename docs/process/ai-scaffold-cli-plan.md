...
```json
{
  ...
  "version": "1.0.0",
  "bin": {
    "ai-scaffold": "bin/ai-scaffold.js"
  }
}
```
...
Equivalent to `npx ai-scaffold init` in the current directory. Installs scaffold-managed files without touching application code.
...
13. `update --version` selects a specific version.
